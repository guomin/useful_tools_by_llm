    ...
    def setup_merge_frame(self, frame):
        ...
        for mode in modes:
            ttk.Radiobutton(mode_frame, text=mode, value=mode, 
                          variable=self.layout_mode).pack(side=tk.LEFT, padx=20, pady=5)
        ...
        ttk.Button(file_btn_frame, text="添加文件", command=self.add_files).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(file_btn_frame, text="移除选中", command=self.remove_selected).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(file_btn_frame, text="上移", command=lambda: self.move_item(-1)).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(file_btn_frame, text="下移", command=lambda: self.move_item(1)).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(file_btn_frame, text="清空列表", command=self.clear_list).pack(side=tk.LEFT, padx=5, pady=5)
        
        # 手动调整面板（默认隐藏）
        self.manual_frame = ttk.LabelFrame(frame, text="手动调整设置")
        
        ttk.Label(self.manual_frame, text="页面尺寸:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.page_size_var = tk.StringVar(value="A4")
        page_sizes = ["A4", "A5", "Letter", "Legal", "自定义"]
        ttk.Combobox(self.manual_frame, textvariable=self.page_size_var, values=page_sizes).grid(row=0, column=1, padx=5, pady=5)
        
        ttk.Label(self.manual_frame, text="宽度(mm):").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.width_var = tk.StringVar(value="210")
        ttk.Entry(self.manual_frame, textvariable=self.width_var).grid(row=1, column=1, padx=5, pady=5)
        
        ttk.Label(self.manual_frame, text="高度(mm):").grid(row=1, column=2, padx=5, pady=5, sticky="w")
        self.height_var = tk.StringVar(value="297")
        ttk.Entry(self.manual_frame, textvariable=self.height_var).grid(row=1, column=3, padx=5, pady=5)
        
        ttk.Label(self.manual_frame, text="页边距(mm):").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.margin_var = tk.StringVar(value="10")
        ttk.Entry(self.manual_frame, textvariable=self.margin_var).grid(row=2, column=1, padx=5, pady=5)
        
        # 合并按钮
        merge_btn_frame = ttk.Frame(frame)
        merge_btn_frame.pack(fill="x", padx=10, pady=10)
        
        ttk.Button(merge_btn_frame, text="合并文件", command=self.merge_files).pack(side=tk.RIGHT, padx=5, pady=5)
